...
public class Hanoi {
  ...
  public static List<List<Integer>> computeTowerHanoi(int numRings) {
    List<Deque<Integer>> pegs = new ArrayList<>(NUM_PEGS);
    for (int i = 0; i < NUM_PEGS; i++) {
      pegs.add(new LinkedList<>());
    }
    for (int i = 0; i < numRings; i++) {
      pegs.get(0).push(numRings - i);
    }
    List<List<Integer>> result = new ArrayList<>();
    return computeTowerHanoiSteps(numRings, pegs, 0, 1, 2, result);
  }

  public static List<List<Integer>> computeTowerHanoiSteps(int numRings, List<Deque<Integer>> pegs, int fromPeg, int toPeg, int usePeg, List<List<Integer>> result) {
    if (numRings > 0) {
      computeTowerHanoiSteps(numRings - 1, pegs, fromPeg, usePeg, toPeg, result);
      pegs.get(fromPeg).addFirst(pegs.get(toPeg).removeFirst());
      result.add(new ArrayList<>(Arrays.asList(fromPeg, toPeg)));
